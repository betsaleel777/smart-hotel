        ...
        if (empty($request->famille) and empty($request->sous_famille)) {
            //le type existe mais famille et sous famille sont vides
                if ($request->type === 'accessoire') {
                    $texte = "WITH r as (SELECT p.libelle,destockages.produit ,destockages.prix, sum(destockages.quantite) as quantite FROM destockages INNER JOIN produits p ON p.id=destockages.produit GROUP BY destockages.produit) SELECT r.prix, r.libelle,a.produit, r.quantite as sortie,sum(a.quantite) as entree FROM approvisionnements a,r WHERE r.produit = a.produit GROUP BY a.produit" ;
                    $query = sprintf($texte);
                } else {
                    $texte = "WITH r as (SELECT p.libelle,restaurations.produit ,restaurations.prix, sum(restaurations.quantite) as quantite FROM restaurations INNER JOIN produits p ON p.id=restaurations.produit GROUP BY restaurations.produit) SELECT r.prix, r.libelle,a.produit, r.quantite as sortie,sum(a.quantite) as entree FROM approvisionnements a,r WHERE r.produit = a.produit GROUP BY a.produit" ;
                    $query = sprintf($texte);
                }
            ...
            $jointure = "INNER JOIN sous_familles s ON p.sous_famille = s.id" ;
                if ($request->type === 'accessoire') {
                    //reste avant
                    $texte = "WITH r as (SELECT p.libelle,destockages.produit ,destockages.prix, sum(destockages.quantite) as quantite FROM destockages INNER JOIN produits p ON p.id=destockages.produit %2\$s WHERE %1\$s  GROUP BY destockages.produit) SELECT r.prix, r.libelle,a.produit, r.quantite as sortie,sum(a.quantite) as entree FROM approvisionnements a,r WHERE r.produit = a.produit GROUP BY a.produit" ;
