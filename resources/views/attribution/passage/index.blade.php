      ...
      <div class="card">
         ...
         <div class="card-body">
            <div class="row">
               ...
               <div class="col-md-2">
                  <a style="width:100%" href="{{ route('attributions_pass_add') }}" class="btn btn-outline-primary"><i
                  ...
                  @foreach ($attributions as $key => $attribution)
                     <tr>
                        ...
                        <td>
                           ...
                           @endif
                           <progression
                              :end="'{{ $attribution->created_at->add($attribution->passageLinked->heure, 'hour') }}'"
                              :chambre="'{{ $attribution->passageLinked->chambreLinked->libelle }}'"></progression>
                        </td>
                        <td>
                           <h5><span
                                 class="{{ $badge[$attribution->passageLinked->chambreLinked->statut] }}">{{ $attribution->passageLinked->chambreLinked->statut }}</span>
                           </h5>
                        </td>
                        <td>
                           @if (!Carbon::now()->greaterThan($attribution->created_at->add($attribution->passageLinked->heure, 'hour')) and $attribution->passageLinked->chambreLinked->statut === 'occupée')
                              @if ($attribution->etat !== 'facturer')
                                 <a href="{{ route('attributions_pass_edit', $attribution) }}"
                                    class="btn btn-outline-success"><i class="fas fa-edit"></i></a>
                                 <a href="{{ route('resto_new', $attribution) }}" class="btn btn-outline-dark"><i
                                       class="fas fa-drumstick-bite"></i></a>
                              @endif
                              <a href="{{ route('destockage_passage_add', $attribution) }}" class="btn btn-outline-dark"><i
                                    class="fas fa-box"></i></a>
                           @endif
                           <a href="{{ route('attributions_pass_liberer', $attribution) }}"
                              class="btn btn-outline-danger"><i class="fas fa-trash"></i>libérer</a>
                        </td>
                     </tr>
                  @endforeach
               </tbody>
