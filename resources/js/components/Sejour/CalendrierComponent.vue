...
export default {
  ...
  methods: {
    ...
    getChambres () {
      axios.get('/ajax/chambres/all/' + this.batiment).then((response) => {
    ...
    prixNet () {
      if (this.message.details) {
        const prix_normal = this.details.type_linked.cout_reservation * this.delais
        const prix_avec_remise = prix_normal - prix_normal * Number(this.remise)
