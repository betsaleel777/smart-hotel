<template>
    ...
    >
        <button type="button" class="btn btn-sm" :class="type">
            {{ "J:" + jours }}
        </button>
        <button type="button" class="btn btn-sm" :class="type">
            {{ "H:" + heures }}
    ...
    mounted() {
        this.timer = setInterval(() => {
            ...
            if (this.temps > 0 && this.temps <= 5 * 60 * 1000) {
                ...
                this.temps > 5 * 60 * 1000 &&
                this.temps <= 15 * 60 * 1000
            ) {
            ...
            if (this.temps >= 0) {
                this.afficher = true;
                this.jours = (
                    "0" + Math.floor(this.temps / (1000 * 60 * 60 * 24))
                ).slice(-2);
                this.heures = (
                    "0" +
                    Math.floor(
                        (this.temps % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60)
                    )
                ).slice(-2);
                this.minutes = (
                    "0" +
                    Math.floor((this.temps % (1000 * 60 * 60)) / (1000 * 60))
                ).slice(-2);
                this.secondes = (
                    "0" + Math.floor((this.temps % (1000 * 60)) / 1000)
                ).slice(-2);
            } else {
                this.afficher = false;
